...
const {
  ...
  eliminarProducto,
  getProducto,
} = require("./../resources/DAOproductos");
const {
  ...
  getTipoProducto,
  getTiposProducto,
} = require("./../resources/DAOtipo_producto");
const {
  getUsuarios,
  crearUsuario,
  actualizarUsuario,
...
const {
  ...
  getOrdenPedido,
  getOrdenesPedido,
} = require("../resources/DAOorden_pedido");
const {
  getReportesFinancieros,
  getReporteFinanciero,
  crearReporteFinanciero,
  actualizarReporteFinanciero,
  eliminarReporteFinanciero,
} = require("../resources/DAOreporte_financiero");
...
const {
  ...
  getDepositosUsuario,
  actualizarDeposito,
} = require("../resources/DAOdepositos");
const jwt = require("jsonwebtoken");
const { error } = require("console");

const CLIENT =
  "AV7RbVPozcoaIgXrxjWQU5WWnGyMyZmMBfauJ16FdFEVU12RTDtFOxSNZzG2GdQUqx5wA6DMwkNR-UfZ";
const SECRET =
  "EOPG-J6D3rZmInvrRvQFuw1N9ZLhOGSEgvKToSaTKBdOltHeXdrsDPNDsui6uT9fyqAAKYpYLUX4p04o";
...
app.get("/", (req, res) => {
  ...
  fs.readFile(
    path.join(__dirname, "..", "public", "doc.html"),
    "utf8",
    (err, data) => {
      if (err) {
        res.send({
...
app.post("/api/depositos", (req, res) => {
  uploader.upload(req, res, (err) => {
    ...
    if (!req.body.monto) {
      ...
      return res.send({
        message: "No se subió ninguna imagen",
        error: "Falta monto del depósito",
      });
    }
    if (req.body.monto < 0) {
      fs.unlink(req.file.path, (err) => {
    ...
    getUsuario(req.body.idUsuario, (error, usuario) => {
      ...
      crearDeposito(deposito, (error) => {
        if (error) {
          return res.send({
            message: "Ocurrió un error al subir o crear comprobante",
            error: error.message,
          });
        }
...
app.delete("/api/depositos/:id", (req, res) => {
  ...
  getDeposito(id, (error, deposito) => {
    if (error) {
      return res.send({
        message: "Error al obtener un depósito",
        error: error.message,
      });
    }
    if (!deposito) {
      return res.send({
        message: "Error al eliminar un depósito",
...
app.put("/api/depositos/:id", (req, res) => {
  ...
  if (!estado) {
    return res.send({
      message: "Error al actualizar el depósito",
      error: "No se proporciono una actualizacion para el estado del deposito",
    });
  }
...
app.delete("/api/productos/:id", (req, res) => {
  ...
  getProducto(id, (error, producto) => {
    if (error) {
      return res.send({
        message: "Error al obtener un producto",
        error: error.message,
      });
    }
    if (!producto) {
...
app.get("/api/tipo-productos/:id", (req, res) => {
  ...
  getTipoProducto(id, (error, tipo_producto) => {
    if (error) {
      return res.send({
        message: "Error al obtener tipo producto",
        error: error.message,
      });
    }
    if (tipo_producto) {
      ...
      res.send({
        message: "Error al obtener tipo producto",
        error: "Tipo producto no encontrado",
      });
    }
  });
});
...
app.post("/api/tipo-productos", (req, res) => {
  if (!req.body.nombreTipo) {
    return res.send({
      ...
      error: "Debe haber un nombre de tipo producto",
    });
  }
...
app.put("/api/tipo-productos/:id", (req, res) => {
  ...
  if (!nombreTipo) {
    return res.send({
      message: "Error al actualizar tipo producto",
      error: "Debe haber un nombre de tipo de producto.",
    });
  }
...
app.delete("/api/tipo-productos/:id", (req, res) => {
  ...
  eliminarTipoProducto(id, (error, success) => {
    if (error) {
      return res.send({
        message: "Error al eliminar tipo producto",
        error: error.message,
      });
    }
    if (success) {
  ...
  autenticarUsuario(correo, contrasenaUsuario, (err, usuario) => {
    if (err) {
      ...
      const token = jwt.sign(
        ...
        "SistemaFerremasTokensDeMierdaAhoraEsPersonal2",
        {
...
app.get("/api/orden-pedido", (req, res) => {
  getOrdenesPedido((error, ordenesPedido) => {
    if (error) {
      res.send({
        message: "Error al obtener ordenes de pedido",
        error: error.message,
      });
    } else {
...
app.get("/api/orden-pedido/:id", (req, res) => {
  ...
  getOrdenPedido(id, (error, ordenPedido) => {
    if (error) {
      res.send({
        message: "Error al obtener orden de pedido",
        error: error.message,
      });
    } else {
      if (ordenPedido) {
...
app.post("/api/orden-pedido", (req, res) => {
  if (!req.body.idUsuario) {
    return res.send({
      message: "Error al crear orden de pedido",
...
app.put("/api/orden-pedido/:id", (req, res) => {
  ...
  actualizarOrdenPedido(id, nuevaOrdenPedido, (error, success) => {
    if (error) {
      res.send({
        message: "Error al actualizar orden de pedido",
        error: error.message,
      });
    } else {
      if (success) {
...
app.delete("/api/orden-pedido/:id", (req, res) => {
  ...
  eliminarOrdenPedido(id, (error, success) => {
    if (error) {
      res.send({
        message: "Error al eliminar orden de pedido",
        error: error.message,
      });
    } else {
      if (success) {
...
const createPayment = (req, res) => {
  ...
  if (req.body.costo < 0) {
    return res.json({
      error: "El costo debe tener un valor minimo de 5 pesos",
    });
  }
  ...
  getUsuario(idUser, (error, usuario) => {
    ...
    fetch(url_dolar)
      ...
      .then((data) => {
        ...
        request.post(
          ...
          (error, response) => {
            if (error) {
              return res.json({
                error: "Error al crear orden de pago: " + error.message,
              });
            }
            ...
            return res.json({
              ...
              paylink: response.body.links[1].href,
            });
          }
        );
      })
      .catch((error) => {
        return res.json({
          error: "Error al obtener el valor del dólar: " + error.message,
        });
      });
  });
};
...
const executePayment = (req, res) => {
  ...
  request.post(
    ...
    (error, response) => {
      if (error) {
        return res.json({
          error: "Error al capturar el pago: " + error.message,
        });
      }
      ...
      const nombreFull =
        response.body.payment_source.paypal.name.given_name +
        " " +
        response.body.payment_source.paypal.name.surname;
      const boleta = {
...
app.get("/api/reportes-financieros", (req, res) => {
  getReportesFinancieros((error, reportes) => {
    if (error) {
      return res.send({
        message: "Error al obtener los reportes financieros",
        error: error,
      });
    }
...
app.get("/api/reportes-financieros/:id", (req, res) => {
  ...
  if (!idReporte) {
    return res.send({
      message: "Error al obtener el reporte financiero",
      error: "Falta id del reporte",
    });
  }
  getReporteFinanciero(idReporte, (err, reporte) => {
    ...
    if (!reporte) {
      return res.send({
        message: "Error al obtener el reporte financiero",
        error: "No se encontro un reporte financiero",
      });
    }
...
app.post("/api/reportes-financieros", (req, res) => {
  ...
  if (!reporte.monto) {
    return res.status(400).send({
      message: "Error al crear el reporte financiero",
      error: "Se necesita un monto",
    });
  }
  ...
  if (!reporte.idUsuario) {
    return res.status(400).send({
      message: "Error al crear el reporte financiero",
      error: "Se necesita el id de usuario del contador",
    });
  }
  ...
  getUsuario(reporte.idUsuario, (error, usuario) => {
    ...
    if (usuario.cargo.toLowerCase() !== "contador") {
      return res.status(403).send({
        message: "Error al crear el reporte financiero",
        error: "Solo los contadores están habilitados para esta acción",
      });
    }
...
app.put("/api/reportes-financieros/:id", (req, res) => {
  ...
  if (!nuevoReporte.monto) {
    return res.status(400).send({
      message: "Error al actualizar el reporte financiero",
      error: "Se necesita un monto",
    });
  }
  ...
  getUsuario(nuevoReporte.idUsuario, (error, usuario) => {
    ...
    actualizarReporteFinanciero(idReporte, nuevoReporte, (error, success) => {
      ...
      if (!success) {
        return res.status(404).send({
          message: "No se encontró ningún reporte financiero con ese ID",
        });
      }
...
app.post("/api/verificar-codigo", (req, res) => {
  ...
  const codigoRecuperacion = codigosRecuperacionList.find(
    (codigo) => codigo.idUsuario === userId
  );

  // Verificar si se encontró un código de recuperación para este usuario
  if (!codigoRecuperacion) {
    return res.json({
      message: "No se encontró un código de recuperación para este usuario",
    });
  }
