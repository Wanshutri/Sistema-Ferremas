...
function validarUsuario(usuario, usuarioAntiguo, callback) {
    ...
    if (usuarioAntiguo != null) {
        ...
        usuarioAntiguo.sNombre = usuarioAntiguo.sNombre ? usuario.sNombre.trim() : null;
    ...
    getUsuarios((error, usuarios) => {
        ...
        if (usuarioExistenteCorreo) {
            if (usuarioAntiguo) {
                if (usuarioAntiguo.correoUsuario !== usuario.correoUsuario) {
                    return callback(new Error('El correo electrónico ya está en uso'), null);
                }
            } else {
                return callback(new Error('El correo electrónico ya está en uso'), null);
            }
        ...
        if (usuario.celular.toString().length !== 9) {
            return callback(new Error('El número de celular debe tener exactamente 9 dígitos'), null);
        }
        ...
        if (usuarioExistenteTelefono) {
            if (usuarioAntiguo) {
                if (usuarioAntiguo.celular !== usuario.celular) {
                    return callback(new Error('El número de celular ya está registrado'), null);
                }
            } else {
                return callback(new Error('El número de celular ya está registrado'), null);
            }
        ...
        if (usuario.direccion.length < 10) {
            return callback(new Error('La dirección debe tener al menos 10 caracteres'), null);
        }
        ...
        if (edad < 18) {
            return callback(new Error('El usuario debe tener al menos 18 años'), null);
        }

        if (usuario.pNombre.length < 2 || usuario.pApellido.length < 2 || usuario.sApellido.length < 2) {
            return callback(new Error('Los nombres y apellidos deben tener al menos 2 caracteres'), null);
        }

        if (usuario.sNombre && usuario.sNombre.length < 2) {
            return callback(new Error('El segundo nombre debe tener al menos 2 caracteres'), null);
        }
...
function actualizarUsuario(id, body, callback) {
    ...
    autenticarUsuario(credenciales.correoUsuario, credenciales.contrasenaUsuario, (error, usu) => {
        if (error) {
            return callback(error, null);
        }
        if (!usu) {
